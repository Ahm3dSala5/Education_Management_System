...
namespace EMS.Core.Features.Departments.Query.Handler
{
         ...
         IRequestHandler<GetDepartmentCourses, Result<List<CourseModel>>>
    {
            ...
            (GetDepartmentByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            return deptMapped != null ?
                Success<DepartmentModel>(deptMapped) :
                BadRequest<DepartmentModel>(_message:"Department Not Found"); 
        }

        public async Task<Result<List<DepartmentModel>>> Handle
            (GetAllDepartmentQuery request, CancellationToken cancellationToken)
        {
            ...
            return departments.Count() != 0 ?
                Success(deptMapped,_meta:$"Number of Department = {deptMapped.Count()}") :
                BadRequest<List<DepartmentModel>>(_message:"Department List Is Empty");
        }

        public async Task<Result<List<InstractorModel>>> Handle
            (GetDepartmentInstractors request, CancellationToken cancellationToken)
        {
        ...
        public async Task<Result<List<StudentModel>>> Handle
            (GetDepartmentStudents request, CancellationToken cancellationToken)
        {
